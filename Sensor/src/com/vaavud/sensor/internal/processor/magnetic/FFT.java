...
public class FFT {
	...
	public FFT(Integer dataLength, Integer FFTLength, Window windowType, 
	      Interpolation interpolationType, Filter filterType, Integer movingAverage) {
	...
	public SensorEvent getSensorEvent(List<SensorEvent> events, Double SF) {
      ...
      List<Double> averageFftResult = averageFftResult(Arrays.<List<Double>>asList(fftResultx, fftResulty, fftResultz));
      ...
      SensorEvent event = new SensorEvent(SensorType.TYPE_FREQUENCY, 
          events.get(events.size()-1).timeUs, new double[]{freqAmp.frequency, freqAmp.amplitude, SF});              
      return event;
    }
...
//					myFAx.frequency, myFAy.frequency, myFAz.frequency,  myFAx.amplitude, myFAy.amplitude, myFAz.amplitude, frequencyRMSD));
  ...
  public Integer getDataLength() {
	...
	private void generatePrecalculatedFilterValues(Filter filterType, Integer movA) {
	  ...
	  if (movA != null) {
	    for (int i =0; i< FFTLength/2; i++) {
	      ...
	      filter[i] = filter[i] * Math.abs(Math.sin(Math.PI*f*movA)/
